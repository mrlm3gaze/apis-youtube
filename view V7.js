...
if (GetData=='' || GetData == undefined || GetData==null) {
	...
	var trendNumber=GetData['trend'],
		...
		array ="",
 		...
 		// language
		if (lg==null||lg==''||lg==undefined||lg=='?:e' || lg!='?:e'&&lg!='?:a')
			{
				lgtrend=' On Trending';
...
function getURL(_order){
	 	...
	 	var url=`https://www.googleapis.com/youtube/v3/commentThreads?part=snippet&maxResults=100&moderationStatus=published&order=${_order}&textFormat=html&videoId=${urlV}&key=${apikey}`,
	 	  ...
	 	  // fetch 
		 	theFech=fetch(req);
					theFech.then(function(response) {
...
function GetStorage(){
			...
			//
			var descText='',
				styleShowMore=document.createElement('style');
			if (desc.length > 200) {
						descText=desc.substring(0,200) +' ...';
						styleShowMore.innerHTML=`
				...
				document.getElementsByTagName('head')[0].appendChild(styleShowMore);	
			//
			moreInfo.innerHTML=`
...
function showMore(d){
	var btnShowHTML=moreInfo.querySelector('.show_hide'),
		btnShow=btnShowHTML.getAttribute('job'),
		descriptionText=moreInfo.querySelector('.DiscTxt');
	if (btnShow=='show') {
		descriptionText.innerHTML=`
			<div class="descText">${d}</div>
			<button class='show_hide' job='hide' onclick='showMore(desc)'>hide</button>	
		`;
	} else {
		descriptionText.innerHTML=`
			<div class="descText">${d.substring(0, 200)+' ...'}</div>
			<button class='show_hide' job='show' onclick='showMore(desc)'>Show More</button>	
		`;
	}
}
...
function myClick(){
  $( document ).ready(function() {
		$('.desc').click(function () {
			...
			// check mode night 
			if (BackGround =='' || BackGround=='0' || BackGround ==undefined || BackGround ==null||BackGround!='1' && BackGround!='0')
			 {
