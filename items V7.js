let ul = document.getElementById("index_tub"),
    ...
    nightMode=0,
	    ...
	    StyleItem=document.createElement('style');
// add new style and select language 
StyleItem.setAttribute('type','text/css')
	    if (lg==null || lg==undefined || lg=='' || lg=='?:e' || lg!='?:e'&& lg!='?:a') {
...
document.getElementsByTagName("head")[0].appendChild(StyleItem);
//added events listener 
document.addEventListener('load',getTube(country));
//added slect
...
function changed(country){
	...
	getTube(select.options[select.selectedIndex].value);
	listItem=[];
}
//get tube maroc
function getTube(country){
	...
	var url=`https://www.googleapis.com/youtube/v3/videos?part=snippet&chart=mostpopular&regionCode=${country}&maxResults=50&key=${apikey}`,
		itemStaticUrl=`https://www.googleapis.com/youtube/v3/videos?part=statistics&chart=mostpopular&regionCode=${country}&maxResults=50&key=${apikey}`,
	    req = new Request(url),
	    results,
	    articls,
	    UrlYoutube;
					...
					// title
	     			...
	     			//descreption lenght > 200 char
	     				if ((articls[i].snippet.description).length > 300) {
	     				 ...
	     				 text=`
	     				<li class="elments" id="${i}">
	     					 <div class='favorite' id="favorite">
	     						<i class="material-icons" id='${i}' job="addToFav">favorite_border</i>
	     		  			</div>
	     			   		<div class="num_tend"> ${i+1} </div>
							<a href="view Video tube.html" title="${title}">
								<h3 class="title_element" id="${i}">${subTitle}</h3>
							</a>
							<div class="Name_Chan">
							...
							<a href="view Video tube.html" id="${i}" title="${title}">
								<div class="img_i">
									  <span class="play" id="${i}">
									  	 <i class="material-icons" id="${i}">play_circle_outline</i>
									  </span>
									  <div style='background:url(${imageUrl}) no-repeat center;background-size:cover' class="img" id="${i}"></div>
								</a>
							...
							<a class="link" href="view Video tube.html" id="${i}" title="${title}"> ${lgWv} </a>
						</li>
	     			`;
	     			ul.insertAdjacentHTML(pos,text);

      				listItem.push({'url':articls[i].id, 'Title':title2, 'tags':tags, 'date':date, 'desc':desc2, 'nameChannel':NmCh, 'LinkChannel':linCod, 'trend':i+1,'poster':imageUrl});
	    		}
	     	})
...
ul.onclick = function ClickItem(event){
 	var Element=event.target,
 		...
 		favorit=Element.getAttribute('job');
 		try {
	 		let url=listItem[Item].url,
	 			...
	 			poster=listItem[Item].poster,

	 			obj={url:url, Title:Title, tags:tags, date:date, desc:desc, nameChannel:nameChannel, LinkChannel:LinkChannel, trend:trend, poster:poster};

	 			sessionStorage.setItem('trend_Data_?Vedio',JSON.stringify(obj));
	 			var dataLocale=JSON.parse(localStorage.getItem('DataForFav'))
	 		if (favorit=='addToFav') {
	 			// favorit=Element.setAttribute('style','color:red')
	 			// favorit=Element.innerHTML='favorite';
	 			// if (dataLocale.includes(Title)) {
	 			// 	favorit=Element.setAttribute('style','color:red')
	 			// 	favorit=Element.innerHTML='favorite';
	 			// } else {
	 			// 	favorit=Element.setAttribute('style','')
	 			// 	favorit=Element.innerHTML='favorite_border';
	 			// }
	 			 alert('Coming soon')
	 		
	 			 // ListFav.push(obj)

 				 // localStorage.setItem('DataForFav',JSON.stringify(ListFav))
...
window.onload = function GetBg() {
	var bg=localStorage.getItem('!Bkg');
	if (bg=='' ||bg==undefined||bg==null || bg=='0' ||bg!='1' && bg!='0') 
	{
		ul.style.background = '#f9f9f9';
		ul.style.color = '#484242';
		vscroll.style.background = '#fff'; 
